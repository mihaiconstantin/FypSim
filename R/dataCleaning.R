...
#' Reconstruct the matrix of results.
#' 
...
#' @param ... (list of raw results) Contains the lists of the raw results as they are returned by \code{runCellRange} or
...
#' First index in the list holds the matrix of detection rates, second the matrix of means, third the matrix
#' of standard deviations, and forth the matrix of effect size measures.
#' 
#' @export
#'
glueDesignPieces <- function(design, ..., saveTo = NULL)
{
	...
	for(iteration in 1:iterations)
	{
		...
		for(piece in pieces)
		{
			...
			temp.effect 	= rbind(temp.effect,     piece[[iteration]][[4]])
		}
		...
		list.effect[[iteration]] 	 = temp.effect
	}
	...
	effect 	   = Reduce("+", list.effect)     / length(list.effect)
	...
	if(!is.null(saveTo))
	{
		...
		saveRDS(effect,     paste(saveTo, "effect_size.RData",         sep = ""))
		...
		write.csv(effect,     paste(saveTo, "effect_size.csv",         sep = ""))
	}
	
	
	# Return the data, just in case.
	return(list(detections, means, deviations, effect))
}
